...
public class PrinterWritingExtention {
	...
	public static void main(String[] args) throws FileNotFoundException {
		// files and stuff
		File skeleton = new File("C:\\Users\\" + user
				+ "\\Documents\\GitHub\\3D-Printer-Writing-Extension\\gcodes\\WitingBaseCode.gcode");
		File text_gcode = new File(
	...
	public static void writeTextFile(Scanner skeletonScanner, double fontSize, PrintStream ps, Scanner textScanner)
	...
	public static void writeUserText(Scanner skeletonScanner, String text, double fontSize, PrintStream ps)
			throws FileNotFoundException {
		// loop to write gcode
		while (skeletonScanner.hasNext()) {
			String nextLine = skeletonScanner.nextLine();
			...
			if (nextLine.equals("; Stuff Go Here")) {
				...
				for (String word : words) {
					...
					if (x + (word.length() * (fontSize * 8 + 2)) > 220) {
						...
						// if the word is still too long, check each letter before going to the next
						// line
	...
	public static void addNewCharacter(character newChar, PrintStream ps) throws FileNotFoundException {
		...
		try {
			gcode = new File(
					"C:\\Users\\" + user + "\\Documents\\GitHub\\3D-Printer-Writing-Extension\\gcodes\\characters\\"
							+ newChar.name + ".gcode");
			gcodeScanner = new Scanner(gcode);
		} catch (Exception FileNotFoundException) {
			gcode = new File("C:\\Users\\" + user
					+ "\\Documents\\GitHub\\3D-Printer-Writing-Extension\\gcodes\\characters\\box.gcode");
			gcodeScanner = new Scanner(gcode);
		}
